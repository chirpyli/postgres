...
/*
 *		xid_age			- 计算 XID 的年龄（相对于最新稳定 xid）
 */
Datum
...
/*
 *		mxid_age			- 计算多事务 XID 的年龄（相对于最新稳定 mxid）
 */
Datum
 ...
 *		XID 的 qsort 比较函数
 *
 * 这仅用于比较来自同一纪元（epoch）的 XID（例如同时运行的各后端）。
 * 因此其中必然都是普通 XID，也就不存在三角不等式的问题。
 */
int
